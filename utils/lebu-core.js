...
let active_axis = 0;
//动态阈值计算
let peak_x_new_max = 0,
    peak_y_new_max = 0,
    peak_z_new_max = 0;
let peak_x_new_min = 0,
    peak_y_new_min = 0,
    peak_z_new_min = 0;
let peak_x_old_max = 0,
    peak_y_old_max = 0,
    peak_z_old_max = 0;
let peak_x_old_min = 0,
    peak_y_old_min = 0,
    peak_z_old_min = 0;
let peak_limit = 50,
    cur_limit = 0;
...
let new_sample_x = 0,
    new_sample_y = 0,
    new_sample_z = 0;
let dynamic_precision = 0.5; //动态精度 
...
//计算配速
function calSpeed(distanceObj, totalTime) {
    let time_split = totalTime.split(":");
    // let hours = time_split[0];
    let minutes = time_split[0];
    let seconds = time_split[1];
    let template = "0.00";
    if (distanceObj > 0 && minutes.length > 0 && seconds.length > 0) {
        // let temp_speed = (parseFloat(minutes) + (parseFloat(seconds) / 60)) / 60;
        console.log(minutes, seconds, 123)
        let temp_speed = (parseFloat(minutes) * 60 + (parseFloat(seconds))) / 3600;
        return speed = (parseFloat(distanceObj) / temp_speed).toFixed(2);
        // let min_speed = Math.floor(temp_speed);
        // let sec_speed = Math.floor((temp_speed - min_speed) * 60.0);
        // speed = min_speed + "'" + sec_speed + "\"";
    } else {
        return speed = template;
    }
}
